...
Тип: трёхмерный графический объект; доступ по хэндлеру; хэндлер представляет собой число (см. выше).
...
Тип: двумерный или трёхмерный источник звука; доступ по хэндлеру; хэндлер представляет собой число (см. выше).
...
Любой оператор записывается на отдельной строке и обязан предваряться целым
положительным числом &mdash; меткой данной строки.

#### Объявление переменных и присваивание

	"LET", expression, "=", expression, line break

В качестве левого `expression` могут использоваться переменные типа число и элементы массивов.

#### Управляющие конструкции

Метка `label` должна быть целым положительным числом, использованным в качестве
метки некоторой строки в программе. Формат метки: `? /[1-9]\d*/ ?`.
...
Комментариями служат строки, начинающиеся с ключевого слова `REM`. Нужно
обратить внимание, что комментарии являются операторами, а потому требуют отдельной строки и наличия метки в самом начале.
