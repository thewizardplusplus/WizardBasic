...
Два оператора не могут иметь одинаковую метку. Метки должны идти в порядке возрастания.
...
Метка `label` должна быть существующей меткой (см. выше) некоторой строки в программе.
