...
std::string Translator::translate(const Parser::ParseTree& parse_tree) {
	...
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::PROGRAM,
		"Wizard BASIC 2: translating error - invalid root node; expected "
		"PROGRAM.");
	...
	parse_tree_node)
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::LINE, "Wizard"
		" BASIC 2: translating error - invalid node; expected LINE.");
	ASSERT(parse_tree_node->children.size() == 1 || parse_tree_node-> children.
		size() == 2, "Wizard BASIC 2: translating error - invalid children "
		"number of node; expected 1 or 2.");

	Parser::ParseTree::const_iterator child = parse_tree_node->children.begin();
	ASSERT(child->value.id() == WizardBasicGrammarRule::
		CONSTANT_POSITIVE_INTEGER, "Wizard BASIC 2: translating error - "
	...
	parse_tree_node)
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		...
		.id() == WizardBasicGrammarRule::FUNCTION_CALL, "Wizard BASIC 2: "
...
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		STATEMENT_ASSIGN, "Wizard BASIC 2: translating error - invalid node; "
	...
	ASSERT(child->value.id() == WizardBasicGrammarRule::IDENTIFIER || child->
		value.id() == WizardBasicGrammarRule::ARRAY_ACCESS, "Wizard BASIC 2: "
...
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		STATEMENT_JUMP, "Wizard BASIC 2: translating error - invalid node; "
		"expected STATEMENT_JUMP.");
	ASSERT(parse_tree_node->children.size() == 1, "Wizard BASIC 2: translating "
...
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		STATEMENT_CONDITION, "Wizard BASIC 2: translating error - invalid "
		"node; expected STATEMENT_CONDITION.");
	ASSERT(parse_tree_node->children.size() == 3, "Wizard BASIC 2: translating "
	...
	ASSERT(type_of_condition == "=" || type_of_condition == "<" ||
		type_of_condition == ">", "Wizard BASIC 2: translating error - invalid "
	...
	const_iterator& parse_tree_node)
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		...
		value.id() == WizardBasicGrammarRule::FUNCTION_CALL, "Wizard BASIC 2: "
...
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		EXPRESSION_ADDITION, "Wizard BASIC 2: translating error - invalid "
		"node; expected EXPRESSION_ADDITION.");
	ASSERT(parse_tree_node->children.size() == 2, "Wizard BASIC 2: translating "
		"error - invalid children number of node; expected 2.");

	std::string type_of_addition = getNodeValue(parse_tree_node);
	ASSERT(type_of_addition == "+" || type_of_addition == "-", "Wizard "
		"BASIC 2: translating error - invalid value of node; expected \"+\" or "
		"\"-\".");
...
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::
		EXPRESSION_MULTIPLICATION, "Wizard BASIC 2: translating error - "
		"invalid node; expected EXPRESSION_MULTIPLICATION.");
	ASSERT(parse_tree_node->children.size() == 2, "Wizard BASIC 2: translating "
		"error - invalid children number of node; expected 2.");

	std::string type_of_multiplication = getNodeValue(parse_tree_node);
	ASSERT(type_of_multiplication == "*" || type_of_multiplication == "/",
		"Wizard BASIC 2: translating error - invalid value of node; expected "
		"\"*\" or \"/\".");
	...
	const_iterator& parse_tree_node)
{
	ASSERT(parse_tree_node->value.id() == WizardBasicGrammarRule::CONSTANT_REAL,
		"Wizard BASIC 2: translating error - invalid node; expected "
		"CONSTANT_REAL.");
